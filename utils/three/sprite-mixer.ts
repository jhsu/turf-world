...
export function SpriteMixer() {
  var actionSprites: any[] = []; // Will store every new actionSprite.
  var listeners: any[] = []; // Will store the user callbacks to call upon loop, finished, etc..
  ...
  function addEventListener(eventName: string, callback: any) {
    if (eventName && callback) {
      listeners.push({ eventName, callback });
    } else {
      throw new Error("Error : an argument is missing");
    }
  }

  // This offsets the texture to make the next frame of the animation appear.
  function offsetTexture(actionSprite: any) {
  ...
  function callFinishedListeners(action: any) {
  ...
  // options.
  function updateAction(action: any, milliSec: number) {
  ...
  function setFrame(frameID: number) {
    this.paused = true;
  ...
  /*
	...
	*/
  function ActionSprite(texture: any, tilesHoriz: number, tilesVert: number) {
    ...
    actionSprite.currentAction = undefined;
  ...
  function Action(
    ...
    if (!actionSprite.isIndexedSprite) {
      throw new Error('Error : "texture" argument must be an indexedTexture.');
    }
